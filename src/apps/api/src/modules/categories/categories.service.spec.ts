...
describe('CategoriesService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [CategoriesService, { provide: PrismaService, useValue: prismaServiceMock }],
    }).compile();
  ...
  describe('create', () => {
    ...
    it('throws ConflictException when category name already exists in travel', async () => {
      ...
      await expect(service.create('travel-1', createDto)).rejects.toThrow(ConflictException);
  ...
  describe('update', () => {
    ...
    it('throws NotFoundException when category does not exist', async () => {
      ...
      await expect(service.update('travel-1', 'non-existent', { name: 'Dining' })).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe('remove', () => {
    ...
    it('throws NotFoundException when category does not exist', async () => {
      ...
      await expect(service.remove('travel-1', 'non-existent')).rejects.toThrow(NotFoundException);
    });
    ...
    it('throws ConflictException when category has expenses', async () => {
      ...
      await expect(service.remove('travel-1', 'cat-1')).rejects.toThrow(ConflictException);
