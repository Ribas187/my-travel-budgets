...
describe('Categories integration tests', () => {
  ...
  describe('update', () => {
    ...
    it('throws ConflictException when updating to a duplicate name', async () => {
      ...
      await expect(categoriesService.update(travel.id, cat2.id, { name: 'Food' })).rejects.toThrow(
  ...
  describe('remove', () => {
    ...
    it('throws ConflictException when category has expenses (409)', async () => {
      ...
      await expect(categoriesService.remove(travel.id, category.id)).rejects.toThrow(
