...
describe('Dashboard integration tests', () => {
  ...
  it('returns correct aggregation totals matching manual sums', async () => {
    ...
    // Member spending
    const alice = result.memberSpending.find(
      (m) => m.memberId === ownerMember.id,
    )
    ...
    // Category spending
    const foodCat = result.categorySpending.find(
      (c) => c.categoryId === food.id,
    )
    const transportCat = result.categorySpending.find(
      (c) => c.categoryId === transport.id,
    )
  ...
  it('includes members with zero expenses with totalSpent: 0', async () => {
    ...
    const guestEntry = result.memberSpending.find(
      (m) => m.memberId === guest.id,
    )
  ...
  it('includes categories with zero expenses with totalSpent: 0', async () => {
    ...
    const cat = result.categorySpending.find(
      (c) => c.categoryId === emptyCategory.id,
    )
  ...
  it('computes alert statuses correctly (ok → warning → exceeded)', async () => {
    ...
    let foodCat = result.categorySpending.find(
    ...
    foodCat = result.categorySpending.find(
      (c) => c.categoryId === category.id,
    )
