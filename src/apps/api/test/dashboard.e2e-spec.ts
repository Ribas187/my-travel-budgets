...
describe('Dashboard E2E', () => {
  ...
  describe('0. Setup fixtures', () => {
    ...
    it('creates categories', async () => {
      ...
      const transportRes = await request(app.getHttpServer())
        ...
        .send({
          ...
          icon: 'car',
          color: '#3498DB',
          budgetLimit: transportBudgetLimit,
        })
        .expect(201);
    ...
    it('creates expenses', async () => {
      ...
      await request(app.getHttpServer())
        ...
        .set('Authorization', `Bearer ${ownerJwt}`)
        .send({
          categoryId: categoryLodgingId,
          amount: ownerLodgingExpense,
          description: 'Hotel',
          date: '2026-07-08',
        })
        .expect(201);
  ...
  describe('1. Authentication & Authorization', () => {
    it('returns 401 without auth token', async () => {
      await request(app.getHttpServer()).get(`/travels/${travelId}/dashboard`).expect(401);
    });
  ...
  describe('2. Dashboard response', () => {
    ...
    it('member spending totals match sum of their expenses', () => {
      ...
      expect(ownerSpending!.totalSpent).toBe(
        ownerFoodExpense1 + ownerFoodExpense2 + ownerLodgingExpense,
      );
      // Member: 50
    ...
    it('category spending totals match sum of expenses per category', () => {
      const categories = dashboard.categorySpending as Array<{
        categoryId: string;
        totalSpent: number;
    ...
    it('overall totalSpent equals sum of all expenses, budget matches travel budget', () => {
      ...
      const expectedTotal =
        ownerFoodExpense1 + ownerFoodExpense2 + memberTransportExpense + ownerLodgingExpense;
