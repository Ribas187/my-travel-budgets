...
describe('Auth & Users E2E', () => {
  ...
  async function verifyAndGetJwt(email: string): Promise<string> {
    ...
    const res = await request(app.getHttpServer()).get('/auth/verify').query({ token }).expect(200);
  ...
  describe('POST /auth/magic-link', () => {
    it('returns 202 for a valid email', async () => {
      ...
      expect(mockSendMagicLink).toHaveBeenCalledWith('valid@example.com', expect.any(String));
    });
    ...
    it('returns 400 when email is missing', async () => {
      await request(app.getHttpServer()).post('/auth/magic-link').send({}).expect(400);
    });
  });
  ...
  describe('GET /auth/verify', () => {
    ...
    it('returns 401 for an expired token', async () => {
      ...
      await request(app.getHttpServer()).get('/auth/verify').query({ token }).expect(401);
    });
    ...
    it('returns 401 for an already-used token', async () => {
      ...
      // Use the token first
      await request(app.getHttpServer()).get('/auth/verify').query({ token }).expect(200);
  ...
  describe('PATCH /users/me', () => {
    ...
    it('returns 401 without authentication', async () => {
      await request(app.getHttpServer()).patch('/users/me').send({ name: 'No Auth' }).expect(401);
    });
  });
