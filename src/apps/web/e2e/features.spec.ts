...
test.describe('Expense edit and delete', () => {
  test('edit modal pre-fills and saves; delete removes from list', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="add-expense-modal"]')).not.toBeVisible({
      timeout: 5000,
    });
    ...
    await expect(page.locator('[data-testid="delete-expense-dialog"]')).not.toBeVisible({
      timeout: 5000,
    });
    await expect(async () => {
...
test.describe('Members page', () => {
  test('shows member list, invite adds member, remove deletes member', async ({ page }) => {
    ...
    await page
      .locator('[data-testid="invite-member-form"]')
...
test.describe('Desktop layout', () => {
  test('sidebar renders with navigation, two-column dashboard layout', async ({
    page,
  }, testInfo) => {
