...
describe('Category Management', () => {
  ...
  describe('useCreateCategory hook', () => {
    ...
    it('invalidates categories.list and dashboard.get queries on success', async () => {
      ...
      expect(queryKeys.categories.list(travelId)).toEqual(['travels', 'travel-1', 'categories']);
      expect(queryKeys.dashboard.get(travelId)).toEqual(['travels', 'travel-1', 'dashboard']);
    });
  });
  ...
  describe('Delete confirmation dialog', () => {
    ...
    it('expense count for a category is computed from expenses list', () => {
      ...
      const count = MOCK_EXPENSES.filter((e) => e.categoryId === categoryId).length;
      expect(count).toBe(2);
    });
  });
  ...
  describe('Empty state', () => {
    it('renders CTA when no categories exist', async () => {
      ...
      expect(i18n.t('category.emptyStateCta')).toBe('Create your first category');
    });
  ...
  describe('Integration: deleting a category removes it from the list', () => {
    it('category is removed from list after deletion', () => {
      ...
      const updatedList = MOCK_CATEGORIES.filter((c) => c.id !== categoryToDelete);
    ...
    it('deleted category no longer appears in the filtered list', () => {
      ...
      const filteredExpenses = MOCK_EXPENSES.filter((e) => e.categoryId !== deletedId);
