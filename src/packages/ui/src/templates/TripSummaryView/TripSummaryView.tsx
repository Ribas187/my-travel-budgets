...
import { StatCard, InsightCard, MemberRow } from '../../molecules';
import {
  formatCurrency,
  formatDateRange,
  formatDate,
...
import type { TravelDetail, DashboardData, Expense, MemberSpending } from '@repo/api-client';
...
export function TripSummaryView({ travel, dashboard, expenses }: TripSummaryViewProps) {
        ...
        {travel.members.map((member, index) => {
          ...
          return (
            <MemberRow
              ...
              amount={formatCurrency(spending?.totalSpent ?? 0, travel.currency, locale)}
            />
          );
        })}
      </YStack>
