...
export function computeTripInsights(
  ...
  if (dashboard.memberSpending.length > 0) {
    ...
    const percentage = totalSpent > 0 ? Math.round((topSpender.totalSpent / totalSpent) * 100) : 0;
  ...
  // Biggest Category
  const categoriesWithSpending = dashboard.categorySpending.filter((cs) => cs.totalSpent > 0);
