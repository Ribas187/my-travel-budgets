...
describe('i18n', () => {
  ...
  describe('new keys completeness', () => {
    const newKeys = [
      // nav
      'nav.profile', 'nav.categories', 'nav.addExpense',
      // dashboard
      'dashboard.budgetHealth', 'dashboard.spent', 'dashboard.remaining',
      'dashboard.avgPerDay', 'dashboard.byCategory', 'dashboard.recentExpenses',
      'dashboard.emptyTitle', 'dashboard.emptyAction',
      // budget
      'budget.title', 'budget.summary', 'budget.totalBudget', 'budget.totalSpent',
      'budget.onTrack', 'budget.overBudgetBy', 'budget.setBudget', 'budget.manage',
      'budget.expenses', 'budget.noBudgetSet',
      // summary
      'summary.title', 'summary.complete', 'summary.totalSpent', 'summary.underBudget',
      'summary.overBudget', 'summary.avgPerDay', 'summary.totalExpenses',
      'summary.budgetUsed', 'summary.insights', 'summary.topSpender',
      'summary.biggestCategory', 'summary.biggestDay', 'summary.perPerson',
      // member (new)
      'member.title', 'member.expenses', 'member.totalSpent', 'member.removeConfirm',
      // profile (new)
      'profile.editName', 'profile.nameSaved', 'profile.languageEn', 'profile.languagePtBr',
      // expense (new)
      'expense.edit', 'expense.updated', 'expense.deleted', 'expense.deleteConfirm',
    ]
  ...
  describe('translate with new keys', () => {
    ...
    it('resolves budget keys with interpolation', () => {
      ...
      expect(translate('pt-BR', 'budget.overBudgetBy', { amount: 'R$50' })).toBe('Acima do orçamento em R$50')
    })
    ...
    it('resolves expense delete confirm with interpolation', () => {
      expect(translate('en', 'expense.deleteConfirm', { description: 'Lunch', amount: '€25' }))
        .toBe('Delete "Lunch" (€25)?')
      expect(translate('pt-BR', 'expense.deleteConfirm', { description: 'Almoço', amount: 'R$50' }))
        .toBe('Excluir "Almoço" (R$50)?')
    })

    it('resolves member removeConfirm with interpolation', () => {
      expect(translate('en', 'member.removeConfirm', { name: 'Ana' }))
        .toBe('Remove Ana from this trip?')
      expect(translate('pt-BR', 'member.removeConfirm', { name: 'Ana' }))
        .toBe('Remover Ana desta viagem?')
    })
  })
